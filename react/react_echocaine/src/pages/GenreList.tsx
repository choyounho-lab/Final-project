...
import { apiGetGenreList, apiGetGenreTrack } from "../api/api";
...
function GenreList() {
  ...
  const genreList: GenreListType[] = [
    ...
    // { id: 25, name: "Film, TV & Stage", color: "#ff3047" },
    ...
    // { id: 36, name: "2000s", color: "#6Ac572" },
    { id: 37, name: "'80s", color: "#FFC857" },
    { id: 38, name: "Afrobeats", color: "#2Ef94E" },
          ...
          <li className="inline-block " key={item.id}>
            {
              <div className="h-100 flex flex-col ">
                <Link to={`/genreListDetail/${item.name.replace(/\s+/g, "")}`}>
                  <div
                    className="m-10 mb-5 w-70 h-70 shadow-xl shadow-gray-600 rounded-xl flex items-end justify-center"
                    style={{ background: `${item.color}` }}
                  >
                    <p className="mx-10 mb-5 font-bold text-2xl line-clamp-1">{`${item.name}`}</p>
                  </div>
                </Link>
              </div>
            }
